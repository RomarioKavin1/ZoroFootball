...
contract Decrypted /*  is IZoroCore, GatewayCaller */ {

    // mapping(uint256 => GameState) public gameStates;
    // mapping (uint256=> EncryptedGameState) public encryptedGameStates;
    // mapping(uint256=>mapping(uint8=>uint8)) public playerGameCards;
    // mapping(uint256=>mapping(uint8=>uint8)) public aiGameCards;
    ...
    // uint8 public index1= 0;
    // uint256 public index2 = 2;
    ...
    //         if(isPlayerCardSlotEmpty[threadId][cardIndex] == true) revert CardSlotEmpty(threadId, cardIndex, true);
    ...
    //         if(isAiCardSlotEmpty[threadId][cardIndex] == true) revert CardSlotEmpty(threadId, cardIndex, false);
    ...
    //         // requestToThreadId[Gateway.requestDecryption(cts, this.processTurnCallback.selector, 0, block.timestamp + 100, false)] = threadId;
    //         _processTurnCallback(2, encryptedState.playerMove, encryptedState.aiMove);
    //     } else {
    ...
    //         uint8 card = uint8((randomNumber ^ encryptedState.playerSeed) % HAND_SIZE);
    ...
    //         uint8 card = uint8((randomNumber ^ encryptedState.aiSeed) % HAND_SIZE);
    ...
    // function _processTurnCallback(uint256 _requestId, uint8 playerMove, uint8 aiMove) internal {
    //     uint256 threadId = requestToThreadId[_requestId];
    //     GameState storage state = gameStates[threadId];
    ...
    // // Helper functions
    // function _initGameSeed() internal returns (uint32, uint32) {
    ...
    // function getPlayerDeck(uint256 threadId) external view returns (uint8[] memory) {
    //     uint8[] memory playerCards = new uint8[](HAND_SIZE);
    //     for(uint8 i = 0; i < HAND_SIZE; i++) {
    ...
    // function getAiDeck(uint256 threadId) external view returns (uint8[] memory) {
    //     uint8[] memory aiCards = new uint8[](HAND_SIZE);
    //     for(uint8 i = 0; i < HAND_SIZE; i++) {
    ...
    // function getPlayerGameSeed(uint256 threadId) external view returns (uint32) {
    //     return encryptedGameStates[threadId].playerSeed;
    // }

    // function getAiGameSeed(uint256 threadId) external view returns (uint32) {
