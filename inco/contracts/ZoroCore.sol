...
contract ZoroCore is IZoroCore, GatewayCaller {
    ...
    function _makeMove(uint256 threadId, uint8 cardIndex, bool isPlayer) public {
        ...
        if (isPlayer) {
            if (state.isPlayerTurn == false) revert NotYourTurn(threadId, state.player);
            ...
            if (state.isPlayerTurn == true) revert NotYourTurn(threadId, aiAgent);
        ...
        if (state.isWaitingForDefence == true) {
            ...
            requestToGameId[
                Gateway.requestDecryption(cts, this.processTurnCallback.selector, 0, block.timestamp + 100, false)
            ] = threadId;
        } else {
            state.isPlayerTurn = state.isPlayerTurn == true ? false : true;
    ...
    function _drawCard(uint256 threadId, uint256 randomNumber, bool isPlayer) public {
        ...
        if (isPlayer) {
            ...
            euint8 card = TFHE.asEuint8(
                TFHE.rem(TFHE.xor(wrappedRandomNumber, encryptedState.playerSeed), uint32(HAND_SIZE))
            );
            ...
            euint8 card = TFHE.asEuint8(
                TFHE.rem(TFHE.xor(wrappedRandomNumber, encryptedState.aiSeed), uint64(HAND_SIZE))
            );
    ...
    function processTurnCallback(uint256 _requestId, uint8 playerMove, uint8 aiMove) external onlyGateway {
        ...
        if (state.currentTurn == MAX_TURNS || state.playerStrikes == 3 || state.aiStrikes == 3) {
            ...
            emit GameEnded(
                threadId,
                state.playerStrikes > state.aiStrikes
                    ? 1
                    : state.aiStrikes > state.playerStrikes
                        ? 0
                        : 2
            );
        }
    ...
    function setCrosschainClient(uint32 _origin, bytes32 _client) external onlyOwner {
        crosschainClients[_origin] = _client;
    }
}
