...
contract ZoroAiClient is IEntropyConsumer {
    ...
    event AiMoveDispatched(uint256 threadId, bytes32 messageId, uint8 cardIndex);
    event AiDrawCardDispatched(uint256 threadId, uint64 sequenceNumber, bytes32 messageId, uint256 randomNumber);

    constructor(IMailbox _mailbox, address _zoroCore, IEntropy _entropy) {
        mailbox = _mailbox;
        owner = msg.sender;
        entropy = _entropy;
        zoroCore = _zoroCore;
    ...
    function makeMove(uint256 threadId, uint8 cardIndex) external onlyAiAgent {
        ...
        bytes32 messageId = mailbox.dispatch{value: 0}(INCO_NETWORK_DOMAIN_ID, addressToBytes32(zoroCore), abi.encode(uint8(2), payload));
        emit AiMoveDispatched(threadId, messageId, cardIndex);
    }

    function drawCard(uint256 threadId, bytes32 randomNumber) external payable onlyAiAgent {
        address provider = entropy.getDefaultProvider();
        uint256 fee = entropy.getFee(provider);
        if(address(entropy) == address(0)) {
            bytes memory payload = abi.encode(aiAgent, threadId, uint256((randomNumber)));
            bytes32 messageId = mailbox.dispatch{value: 0}(INCO_NETWORK_DOMAIN_ID, addressToBytes32(zoroCore), abi.encode(uint8(1), payload));
            emit AiDrawCardDispatched(threadId, 0, messageId, uint256(randomNumber));
        }else{
    ...
    function entropyCallback(uint64 sequenceNumber, address, bytes32 randomNumber) internal override {
        ...
        bytes memory payload = abi.encode(aiAgent, _threadId, uint256(randomNumber));
        bytes32 messageId = mailbox.dispatch{value: 0}(INCO_NETWORK_DOMAIN_ID, addressToBytes32(zoroCore), abi.encode(uint8(1), payload));
        emit AiDrawCardDispatched(_threadId, sequenceNumber, messageId, uint256(randomNumber));
    }
